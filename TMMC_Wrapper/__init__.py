    ...
    def start_keyboard_control(self):
        if self.keyboard_listener is None:
            def on_press(key):
                ...
                min_dist, min_dist_angle = self.detect_obstacle(self.last_scan_msg.ranges)
           #     print("Distance to Wall: " + str(min_dist))
            #    print("Current Rotation: " + str(min_dist_angle))

                if key_char == 'w':
                    #if there's an obstacle 0.15m in FRONT of the robot, stop moving
                    if (min_dist_angle > -20 and min_dist_angle < 20) and (min_dist != -1 and min_dist < 0.7):
                        self.send_cmd_vel(0.0, 0.0) #stop the robot's motors
                        #self.move_backward() #move the robot backwards
                        print("STOPPED")
                        return
                    self.move_forward()
                if key_char == 's':
...
# stop sign
    ...
    def stop_sign_STOP(self):
        self.checkImageRelease()
        img = self.rosImg_to_cv2()
        lower_red = (0, 0, 200)
        upper_red= (128, 128, 255)
